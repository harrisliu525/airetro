...
export async function POST(req: NextRequest) {
  ...
  const { prompt, imageUrls } = (await req.json()) as GenerateImageRequest;

  const sanitizedPrompt = prompt?.trim();
  const sanitizedUrls = Array.isArray(imageUrls)
    ? imageUrls.map((url) => url?.trim()).filter(Boolean)
    : [];

  if (!sanitizedPrompt || sanitizedUrls.length < 2) {
    const error = 'Missing prompt or reference image URLs';
  ...
  const payload: ApicoreImagePayload = {
    prompt: `${sanitizedUrls.join(' ')} ${sanitizedPrompt}`.trim(),
    model: APICORE_MODEL,
